...
class VocabCalendarModal extends Modal {
    ...
    async onOpen() {
        ...
        // Make the built-in close button round to match pill design
        const closeBtn = this.modalEl.querySelector('.modal-close-button') as HTMLElement;
        if (closeBtn) closeBtn.addClass('vocab-modal-close-btn');
...
export default class ZenVocabAIPlugin extends Plugin {
    ...
    // ─── Image Code Mnemonic ─────────────────────────────
    async loadMnemonicCodeLibrary(): Promise<string> {
        if (!this.settings.mnemonicEnabled) return '';
        const path = normalizePath(this.settings.mnemonicCodePath || "Vocab/Image_Code_Library.md");
...
class ZenVocabSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        // ── Image Code Mnemonic ─────────────────────────
        containerEl.createEl('h3', { text: '🧩 图像代码法 | Image Code Mnemonic', cls: 'vocab-settings-header' });

        new Setting(containerEl)
            .setName('启用图像代码法 | Enable Image Code')
            .setDesc('在 AI 词汇解析中添加"方法E：图像代码法"记忆路径')
            .addToggle(toggle => toggle
                .setValue(this.plugin.settings.mnemonicEnabled)
                .onChange(async (value) => {
        ...
        new Setting(containerEl)
            .setName('代码库文件路径 | Code Library Path')
            .setDesc('图像代码库在 Obsidian 库中的位置，供 API 和手动查阅使用')
            .addText(text => text
                .setValue(this.plugin.settings.mnemonicCodePath)
                .setPlaceholder('Vocab/Image_Code_Library.md')
                .onChange(async (value) => {
